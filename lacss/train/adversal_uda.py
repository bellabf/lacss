...
class Discriminator(nn.Module):
    ...
    def __call__(self, features):
        ...
        y = nn.LayerNorm()(x)
        token = self.param("token", nn.initializers.normal(stddev=0.02), y.shape[:-2] + (1, y.shape[-1]))
...
class LacssAdv(nn.Module):
    ...
    def __call__(self, image, gt_locations=None, **kwargs):
        det_feature, seg_feature = gradient_reversal(self.lacss.get_features(image, None, deterministic=False))
...
class AdversalUDA(VMapped):
    ...
    def train_step(
        ...
        if label is not None:
            ...
            grads = jax.tree_util.tree_map(lambda a,b: a * cls.beta + b, uda_grad['lacss'], grads)
